...
In this section we discuss the various components of Seraphis, with security theorems and proofs introduced where appropriate. Seraphis is an {\em abstract} protocol, so various implementation details are left undefined. In particular, an implementer needs to define the generators $G_0, G_1, H_0, H_1, X$ (see Section \ref{subsec:implementers-generators} for example), specify a membership proof structure (e.g.\ Appendix \ref{appendix:membership-with-grootle}), specify an unspentness/ownership proof structure (e.g.\ Appendix \ref{appendix:composition-with-schnorr}), and define how to check that transaction amounts balance (e.g.\ Section \ref{subsec:implementers-amount-balancing}).
