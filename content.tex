...
\begin{enumerate}
    ...
    \item An observer who knows none of $x, y, x G$ or $y U$ should be able to derive $y U$ from the resulting proof.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Flexible membership proof delegation}: Rather than randomly generating $t_{c,i}$ and $t_{k,i}$, it may be better to generate them in the random oracle model, such as $t_{c,i} = \mathcal{H}_c(k^{vr}, k^{vs}, K^o, C)$ and $t_{k,i} = \mathcal{H}_k(k^{vr}, k^{vs}, K^o, C)$. This way, for example, a tier 2 wallet can construct membership proofs independently of a tier 3 wallet that focuses on constructing/signing ownership/unspentness proofs.
\end{itemize}
