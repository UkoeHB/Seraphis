...
Now the user is prepared to recover the enote amount.\footnote{These steps are not required for coinbase enotes, which record the amount $a$ explicitly.} Selfsend scanning will test a trial selfsend type $\tau$. In selfsend scanning, if one $\tau$ fails then other values of $\tau$ can be tried.
