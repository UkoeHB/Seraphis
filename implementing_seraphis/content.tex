...
\begin{itemize}
    ...
    \item \textbf{Find-received pubkey}: $K_{fr} = k_{fr} K_{ua}$
\end{itemize}
...
\begin{align*}
    K^j_1 &= k^j_g G + k^j_x X + k^j_u U + K_s \\
    K^j_2 &= k^j_a * K_{fr} \\
    K^j_3 &= k^j_a * K_{ua}
\end{align*}

The index is ciphered as $c^j = {\tt Ciph}[s_{ct}](j)$ and prepended to a `decipher hint' to give the address tag:\footnote{The Jamtis address tag decipher hint only needs to be 1-2 bytes in size to be effective.}
\[ \textrm{\tt addr\_tag}^j = c^j\textrm{ $||$ }\mathcal{H}_{dch}(s_{ct} || c^j) \]

A full Jamtis public address is the tuple $[K^j_1, K^j_2, K^j_3, \textrm{\tt addr\_tag}^j]$.
...
\subsubsection{Diffie-Hellman derived key $K_d$}
...
For most enotes, $K_e$ and $K_d$ will be computed directly from the recipient's address keys.\vspace{.115cm}
\begin{align*}
    K_e &= r K^j_3 \\
    K_d &= r K^j_2
\end{align*}

In this case, $K_d = r K^j_2 = r k_{fr} K^j_3 = k_{fr} K_e$.
...
To further ensure uniqueness {\em within} a transaction, transaction verifiers must mandate that all values $K_e$ in a transaction are unique (with a further caveat for selfsend enotes mentioned in Section \ref{subsubsec:jamtis-srsecret-derivation-selfsend}).\footnote{It is safe for multiple $K_e$ in a transaction to be cofactor-variants of the same point, since we bind $s^{sr}_1$ to the byte-wise representation of $K_e$.}

\subsubsection{Secrets $s^{sr}_1, s^{sr}_2$ (derivation path 1): normal enotes}
\label{subsubsec:jamtis-srsecret-derivation-normal}

For normal enotes that transfer funds to another person, the two sender-receiver secrets $s^{sr}_1, s^{sr}_2$ are derived from $K_d$ and $r$.

\textbf{Sender}
\begin{align*}
    s^{sr}_1 &= \mathcal{H}_{sr1n}(K_d || K_e || \texttt{input\_context}) \\
...
The second secret $s^{sr}_2$ is computed from an `inverse' Diffie-Hellman exchange involving the recipient's address key $K^j_3 = (k^j_a * k_{ua})*G$. The usefulness of $s^{sr}_2$ will be explored in Sections \ref{subsubsec:jamtis-cryptonote-flaws-janus} and \ref{subsec:jamtis-wallets}.
...
As we will see in Section \ref{subsubsec:jamtis-wallets-remote-assisted}, providing a separate derivation path for selfsend enotes improves the privacy attributes of balance recovery when $k_{fr}$ is given to a semi-trusted third party.
...
\textbf{Optimized design}: Selfsend enote secrets can be computed with three hash operations (including the input context), whereas normal enote secrets require three hash operations, one scalar inversion, and two ECDH exchanges.


\subsection{Jamtis enote construction}
\label{subsec:jamtis-enote-construction}

Jamtis enotes are constructed in a linear fashion from $K_d, s^{sr}_1$, and $s^{sr}_2$.
...
\begin{align*}
    K^o = \mathcal{H}^n_{kog}(K^j_1 || s^{sr}_1 || C)*G &+ \\
    \mathcal{H}^n_{kox}(K^j_1 || s^{sr}_1 || C)*X &+ \\
    \mathcal{H}^n_{kou}(K^j_1 || s^{sr}_1 || C)*U &+ K^j_1
\end{align*}

We will justify the use of three extensions in Section \ref{subsec:jamtis-forward-secrecy-dlp-solver}. We explain why binding the extensions to $K^j_1$ is important in Appendix \ref{appendix:jamtis-information-proofs-enote-ownership}. Onetime addresses do {\em not} bind to $s^{sr}_2$ so they may be efficiently recovered with only one ECDH exchange using $k_{fr}$ (see Section \ref{subsec:jamtis-balance-recovery}).
...
\subsubsection{View tag}
\label{subsubsec:jamtis-enote-construction-view-tag}

As a balance recovery optimization, we include a MAC-like hash of $K_d$ called a `view tag' in enotes \cite{reduce-scan-times-view-tag-research-issue-73}. A person doing balance recovery can recompute the view tag for a given enote and check it against the stored view tag. If the view tags don't match then all other balance recovery steps can be skipped (the other steps are quite expensive in total, compared to the ECDH exchange for $K_d$).\footnote{A view tag only needs to be 1 or 2 bytes to effectively amortize post-view-tag computations.}
\[ {\tt view\_tag} = \mathcal{H}_{vt}(K_d || K^o) \]
...
A Jamtis enote sending funds to an address $[K^j_1, K^j_2, K^j_3, \textrm{\tt addr\_tag}^j]$ is therefore the tuple\footnote{A Jamtis coinbase enote will record $a$ explicitly instead of $C$ and $a_{enc}$.}\vspace{.115cm}
...
CryptoNote balance recovery involves a two-part process. First is an ECDH exchange using the CryptoNote view key to get a shared secret (much like $K_d$). Then the shared secret is used to unwrap the spend key of the address that owns the enote (similar to $K^j_1$).
...
\begin{enumerate}
    \item The enote ephemeral key $K_e$ depends on $k^j_a$: $K_e = r K^j_3 = r*(k^j_a * k_{ua})*G$

    \item The recipient recovers index $j$ by decrypting {\tt addr\_tag\_enc} with $K_d = k_{fr} K_e$ (and then deciphering the decrypted address tag).

    \item The recipient recomputes onetime address $K^o$ using $K_s$, $j$, and $K_d$.

    \item The recipient computes the second secret $s^{sr}_2$ from the value $(1/(k^j_a * k_{ua})) * K_e$.
\end{enumerate}

The key here is in the last point. Specifically, $s^{sr}_2$ depends on both $K_e$ and the recovered $j$. If the recovered $j$ does not match the $j'$ of the key $K^{j'}_3$ used to construct $K_e = r K^{j'}_3$, then the user will compute a different $s^{sr}_2$ from the transaction author.
...
\subsubsection{View tag check and $K_d$}
\label{subsubsec:jamtis-balance-recovery-view-tag}

Given an enote with associated enote ephemeral key $K_e$ and {\tt input\_context}, a user who wants to see if they own the enote begins by checking its view tag. This requires the user's $k_{fr}$.

\begin{enumerate}
    \item Compute the nominal key derivation: $K'_d = k_{fr} * K_e$

    \item Compute the nominal view tag: ${\tt view\_tag}' = \mathcal{H}_{vt}(K'_d || K^o)$
...
Once an enote has been flagged as `possibly owned', full enote recovery can proceed. Normal scanning will use the key derivation $K'_d$, while selfsend scanning will test a trial selfsend type $\tau$.
...
\begin{itemize}
    \item \textbf{Normal scan}: This requires $K'_d$ and the user's $s_{ct}$.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Normal scan}: $s'^{sr}_1 = \mathcal{H}_{sr1n}(K'_d || K_e || \texttt{input\_context})$
    ...
    \item Split the nominal address tag: ${\tt addr\_tag}' \Rightarrow \{c'^j, {\tt hint}'\}$

    \item If $\mathcal{H}_{dch}(s_{ct} || c'^j) \neq {\tt hint}'$ then {\tt ABORT}.
...
\begin{itemize}
    \item \textbf{Find-received} ($k_{fr}$): Perform view tag checks  for all enotes and compute nominal normal-enote address tags.

    \item \textbf{Generate-address} ($s_{ga}, K_s$): Generate any Jamtis address for arbitrary index $j$. Decipher the address index of any existing Jamtis address.

    \item \textbf{Payment validator} ($k_{fr}, s_{ga}, k_{ua}, K_s$): In addition to the find-received and generate-address tier capabilities, the payment validator can perform balance recovery for normal enotes up to (but not including) the key image computation.
...
\begin{itemize}
    \item The adversary can solve for $k_{fr}$ in $k_{fr} K^j_3 = K^j_2$ and use it to unwrap candidate onetime addresses to check if the unwrapped spend keys match the address's $K^j_1$.

    \item The adversary can solve for $r$ in $r K^j_3 = K_e$ to compute the key $r G$, which allows the $s^{sr}_2$ of normal enotes to be computed.
\end{itemize}
