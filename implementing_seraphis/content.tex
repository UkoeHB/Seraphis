...
As a balance recovery optimization, we include two MAC-like hashes of $D^d_{fa}$ and $D^d_{vr}$ called the `primary view tag' and `complementary view tag' in enotes \cite{reduce-scan-times-view-tag-research-issue-73}. For auxiliary selfsend enotes, we also include an `auxiliary view tag`. These MAC-like hashes are spliced together into a fixed-size byte buffer called the `view tag'. To make the view tag, we concatenate a certain number of bits, ${\tt npbits}$, from the primary view tag, to another ${\tt ncbits}$ from the complementary view tag. A person doing balance recovery can recompute the primary/complementary/auxiliary view tags for a given enote and check them against the stored aggregate view tag. If the computed sub-tags don't match against their bit ranges in the aggregate tag, then most of the other balance recovery steps, especially the most expensive steps, can be skipped.\footnote{Each primary/complementary view tag only needs to be 1 or 2 bytes to effectively amortize post-view-tag computations.}

\textbf{Normal Enotes and Exclusive Selfsend Enotes}

For all enotes that aren't auxiliary selfsend enotes, we compute the individual parts that make up an aggregate view tag like this:\vspace{.115cm}
\begin{align*}
...
Putting it all together we get\vspace{.115cm}
