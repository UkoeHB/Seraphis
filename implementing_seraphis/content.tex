...
From the view-balance key we derive the following:\footnote{The Jamtis keys $k_{ua}, k_{dv}, k_{sv}, D^j_{ua}, D^j_{dv}, D^j_{sv}$ may be defined on a separate elliptic curve from $\mathbb{G}$, for example with X25519 for maximized ECDH performance. For simplicity in this document we assume the primary curve $\mathbb{G}$ is used. If those keys are defined on a group with cofactor $h > 1$, then the cofactor should be included as a scalar multiplier in sender receiver secrets, for example $K^{dv}_d = h * k_{dv} * K_e$. If this is not done, then a malicious transaction author can determine if the recipient's relevant ECDH secret key is a multiple of $h$ (or any divisor of $h$) or not. They only need to set, for example, $K_e = K_e + K^{hf}$, where $K^{hf}$ is a point in the subgroup of order $h$ (or a factor of $h$). If the recipient successfully performs balance recovery on the enote and notifies the sender, then the sender will know that $k_{dv}$ is a multiple of the subgroup order of $K^{hf}$. This is because $k_{dv} * K^{hf} == I$ when $k_{dv}$ is a multiple of $K^{hf}$'s subgroup's order, allowing the recipient to successfully reproduce $K^{dv}_d$ only in that case. Including $h$ in sender-receiver secrets means cofactor-subgroup points will always be `canceled out' if they are present in ECDH base keys.}
\begin{itemize}
    \item \textbf{Unlock-amounts key}: $k_{ua} = \mathcal{H}^n_{ua}(k_{vb})$
    \item \textbf{Dense-view key}: $k_{dv} = \mathcal{H}^n_{dv}(k_{vb})$
    \item \textbf{Sparse-view key}: $k_{sv} = \mathcal{H}^n_{sv}(k_{vb})$
    \item \textbf{Generate-address secret}: $s_{ga} = \mathcal{H}_{ga}(k_{vb})$
...
\begin{itemize}
    ...
    \item \textbf{Unlock-amounts pubkey}: $K_{ua} = k_{ua} G$
    \item \textbf{Dense-view key pubkey}: $K_{dv} = k_{dv} K_{ua}$
    \item \textbf{Sparse-view pubkey}: $K_{sv} = k_{sv} K_{ua}$
\end{itemize}
...
\[ k^j_a = \mathcal{H}^n_{ap}(K_s || j || s^j_{gen}) \]
...
The index is ciphered as $\textrm{\tt addr\_tag}^j = {\tt Ciph}[s_{ct}](j)$.

A full Jamtis public address is the tuple $[K^j_s, D^j_{ua}, D^j_{dv}, D^j_{sv}, \textrm{\tt addr\_tag}^j]$.
...
As we will see, an enote owner can always use their dense-view key $k_{dv}$ with $K_e$ to recover the Diffie-Hellman derived key $K^{dv}_d$:
\[ K^{dv}_d = k_{dv} K_e \]

Likewise, an enote owner can always use their sparse-view key $k_{sv}$ with $K_e$ to recover the Diffie-Hellman derived key $K^{sv}_d$:
\[ K^{sv}_d = k_{sv} K_e \]

There are two possible ways to compute $K_e$, $K^{dv}_d$, and $K^{sv}_d$ during transaction construction.

\textbf{Standard}

For most enotes, the sender will compute $K_e$, $K^{dv}_d$, and $K^{sv}_d$ directly from the recipient's address keys.\vspace{.115cm}
\begin{align*}
    K_e      &= r D^j_{ua} \\
    K^{dv}_d &= r D^j_{dv} \\
    K^{sv}_d &= r D^j_{sv}
...
In this case, you can see that the sender and receiver will calculate the same DH keys:\vspace{.115cm}
\begin{align*}
    K^{dv}_d = r D^j_{dv} = r * k_{dv} * D^j_{ua} = k_{dv} K_e. \\
    K^{sv}_d = r D^j_{sv} = r * k_{sv} * D^j_{ua} = k_{sv} K_e.
\end{align*}

\textbf{Shared-$K_e$ optimization}
...
In this case, the transaction author knows his own $k_{dv}$ and $k_{sv}$ keys, so he can define the following:\vspace{.115cm}
\begin{align*}
    K^{\textrm{self}}_e &= K^{\textrm{other}}_e \\
    K^{dv}_d            &= k_{dv} * K^{\textrm{self}}_e \\
    K^{sv}_d            &= k_{sv} * K^{\textrm{self}}_e
\end{align*}

which ensures he can properly recompute $K^{dv}_d$ and $K^{sv}_d$ during balance recovery.

A transaction that uses the shared-$K_e$ optimization only needs to store one copy of $K_e$.\footnote{A major advantage of sharing $K_e$ is you only need to derive $K^{dv}_d$ and $K^{sv}_d$ once instead of twice. If most transactions in the ledger have two outputs, and all two-output transactions use the shared-$K_e$ optimization, then there will be a substantial reduction in $K^{dv}_d$ and $K^{sv}_d$ computations relative to a scenario with no $K_e$ sharing.}
...
To further ensure uniqueness {\em within} a transaction, transaction verifiers must mandate that all values $K_e$ in a transaction are unique (with a further caveat for selfsend enotes mentioned in Section \ref{subsubsec:jamtis-srsecret-derivation-selfsend}).\footnote{It is safe for multiple $K_e$ in a transaction to be cofactor-variants of the same point, since we bind $s^{sr}_{1}$ to the byte-wise representation of $K_e$.}

\subsubsection{Secrets $s^{sr}_1, s^{sr}_2$ (derivation path 1): normal enotes}
\label{subsubsec:jamtis-srsecret-derivation-normal}

For normal enotes that transfer funds to another person, the two sender-receiver secrets $s^{sr}_1, s^{sr}_2$ are derived from $K^{dv}_d$, $K^{sv}_d$, and $r$.

\textbf{Sender}
\begin{align*}
    s^{sr}_1 &= \mathcal{H}_{sr1n}(K^{dv}_d || K^{sv}_d || K_e || \texttt{input\_context}) \\
...
\begin{align*}
    s^{sr}_1 &= \mathcal{H}_{sr1n}(K^{dv}_d || K^{sv}_d || K_e || \texttt{input\_context}) \\
    s^{sr}_2 &= \mathcal{H}_{sr2n}((1/(k^j_a * k_{ua})) * K_e)
\end{align*}

The second secret $s^{sr}_2$ is computed from an `inverse' Diffie-Hellman exchange involving the recipient's address key $D^j_{ua} = (k^j_a * k_{ua})*G$. The usefulness of $s^{sr}_2$ will be explored in Sections \ref{subsubsec:jamtis-cryptonote-flaws-janus} and \ref{subsec:jamtis-wallets}.
...
When sending funds to himself, a transaction author derives the two sender-receiver secrets $s^{sr}_1, s^{sr}_2$ from $K_e$ and his Jamtis key $k_{vb}$. The secrets can be recomputed in the same way during balance recovery.

As we will see in Section \ref{subsubsec:jamtis-wallets-remote-assisted}, providing a separate derivation path for selfsend enotes improves the privacy attributes of balance recovery when receiving secrets are given to a semi-trusted third party.

It is possible to define multiple different selfsend enote types (e.g.\ change enotes, self-spend enotes, dummy selfsend enotes). For convenience, we domain-separate $s^{sr}_1$ based on the different selfsend enote types $\tau$.
\begin{align*}
    s^{sr}_1 &= \mathcal{H}_{sr1s[\tau]}(k_{vb} || K_e || {\tt input\_context}) \\
    s^{sr}_2 &= \mathcal{H}_{sr2s}(k_{vb} || s^{sr}_1)
\end{align*}

In this case $s^{sr}_2$ does not rely on the inverse Diffie-Hellman secret $r G$, which simplifies the shared-$K_e$ optimization because there is no need for the value $(1/(k^j_a * k_{ua})) * K^{\textrm{other}}_e$ when building the shared-$K_e$ selfsend enote.

\textbf{Uniqueness}: To ensure uniqueness of $s^{sr}_1$ and $s^{sr}_2$ when $K_e$ is shared between two enotes, transaction authors must avoid sharing $K_e$ between two selfsend enotes of the same type. The secrets will be unique in all other cases since the shared-$K_e$ enote is a selfsend, and each selfsend enote type's secrets are domain-separated from secrets of all other enotes types.

\textbf{Optimized design}: Selfsend enote secrets can be computed with three hash operations (including the input context), whereas normal enote secrets require four hash operations, one scalar inversion, and three ECDH exchanges.


\subsection{Jamtis enote construction}
\label{subsec:jamtis-enote-construction}

Jamtis enotes are constructed in a linear fashion from $K^{dv}_d, K^{sv}_d, s^{sr}_1$, and $s^{sr}_2$.
...
Enote addresses are called `onetime addresses' because they can only be used to produce one key image, and hence should only appear once in the ledger. An enote's onetime address is:\vspace{.115cm}
\begin{align*}
...
\[ {\tt addr\_tag\_enc} = {\tt addr\_tag\ XOR  } \mathcal{H}_{aie}(s^{sr}_1 || K^o) \]

We bind to $K^o$ for robustness.\footnote{It is feasible, although unlikely, that $s^{sr}_1$ could be re-used between separate transaction attempts using different recipients. For example, a shared-$K_e$ selfsend could use two different selfsend addresses but the same original recipient and same enote ephemeral private key $r$. In that case, binding to $K^o$ ensures different address tags won't be encrypted using the same secret.}

\subsubsection{View tags}
\label{subsubsec:jamtis-enote-construction-view-tag}

As a balance recovery optimization, we include MAC-like hashes of $K^{dv}_d$ and $K^{sv}_d$ called `view tags' in enotes \cite{reduce-scan-times-view-tag-research-issue-73}. A person doing balance recovery can recompute the view tags for a given enote and check them against the stored view tags. If the view tags don't match then all other balance recovery steps can be skipped (the other steps are quite expensive in total, compared to the ECDH exchanges for $K^{dv}_d$ and $K^{sv}_d$). The reason for the two different levels of view tags is so that a user has the ability to give different actors different levels of probabilistic deanonymization to offload scanning work. If a receiver only has access to $k_{dv}$ or $k_{sv}$, they can only compute dense or sparse view tags, respectively. The most notable example where this could be used is in a ``lite wallet server'' where users give $k_{dv}$ or $k_{dv}$ (but not both) to a third-party to scan enotes in the background for them to massively reduce refresh times when they open their wallet application. \footnote{A view tag only needs to be 1 or 2 bytes to effectively amortize post-view-tag computations. A good choice which is used in the implementation is 1 byte for the dense view tag, and 2 bytes for the sparse view tag. Combined, checking these two tags will filter out all but 1:16777216 enotes on average. }
\[ {\tt dense\_view\_tag} = \mathcal{H}_{dvt}(K^{dv}_d || K^o) \]
\[ {\tt sparse\_view\_tag} = \mathcal{H}_{svt}(K^{sv}_d || K^o) \]

We bind to $K^o$ in order to transitively bind to all other components of an enote, ensuring the view tag derivations will never be repeated between two enote construction events. We highlight the advantage of using two view tags, rather than one, in Section \ref{subsubsec:jamtis-balance-recovery-view-tag}.

\subsubsection{Enote summary}
\label{subsubsec:jamtis-enote-construction-summary}

A Jamtis enote sending funds to an address $[K^j_s, D^j_{ua}, D^j_{dv}, D^j_{sv}, c^j]$ is therefore the tuple\footnote{A Jamtis coinbase enote will record $a$ explicitly instead of $C$ and $a_{enc}$.}\vspace{.115cm}
\[ [K^o, C, a_{enc}, {\tt addr\_index\_enc}, {\tt dense\_view\_tag}, {\tt sparse\_view\_tag}] \]

The enote is associated with an ephemeral public key $K_e$ and an {\tt input\_context}.
...
CryptoNote balance recovery involves a two-part process. First is an ECDH exchange using the CryptoNote view key to get a shared secret (much like $K_d$). Then the shared secret is used to unwrap the spend key of the address that owns the enote (similar to $K^j_s$).
...
\begin{enumerate}
    \item The enote ephemeral key $K_e$ depends on $k^j_a$: $K_e = r D^j_{ua} = r*(k^j_a * k_{ua})*G$

    \item The recipient recovers index $j$ by decrypting {\tt addr\_index\_enc} with $K^{dv}_d = k_{dv} K_e$ and $K^{sv}_d = k_{sv} K_e$ (and then deciphering the decrypted address tag).

    \item The recipient recomputes onetime address $K^o$ using $K_s$, $j$, $K^{dv}_d$, and $K^{sv}_d$.

    \item The recipient computes the second secret $s^{sr}_2$ from the value $(1/(k^j_a * k_{ua})) * K_e$.
\end{enumerate}

The key here is in the last point. Specifically, $s^{sr}_2$ depends on both $K_e$ and the recovered $j$. If the recovered $j$ does not match the $j'$ of the key $D^{j'}_{ua}$ used to construct $K_e = r D^{j'}_{ua}$, then the user will compute a different $s^{sr}_2$ from the transaction author.

In detail, the user will compute $s^{sr}_2$ with the ECDH key $(1/(k^j_a * k_{ua})) * K_e = r * (k^{j'}_a/k^j_a) * G$, which the transaction author cannot know since $k^{j}_a$ and $k^{j'}_a$ are secrets. Therefore if a user recovers $s^{sr}_2$ successfully (which can be verified by reproducing $C$ correctly), and assuming $K^o$ was recomputed properly, then the transaction author must have used address components derived from the same index $j$.
...
\subsubsection{View tag checks and $K^{dv}_d$ / $K^{sv}_d$}
\label{subsubsec:jamtis-balance-recovery-view-tag}

Given an enote with associated enote ephemeral key $K_e$ and {\tt input\_context}, a user who wants to see if they own the enote begins by checking its view tags. If a user has access to the sparse view key $k_{sv}$, it is recommened that they check against this view tag first, since it tends to be larger, and thus filters out more computation.

\begin{enumerate}
    \item Compute the nominal key derivation: $K'^{sv}_d = k_{sv} * K_e$

    \item Compute the nominal view tag: ${\tt sparse\_view\_tag}' = \mathcal{H}_{svt}(K'^{sv}_d || K^o)$

    \item If ${\tt sparse\_view\_tag}' \neq {\tt sparse\_view\_tag}$ then {\tt ABORT}.
\end{enumerate}

If this task succeeds, or if you were unable to perform it in the first place due to not knowing $k_{sv}$, move onto the dense view tag check:

\begin{enumerate}
    \item Compute the nominal key derivation: $K'^{dv}_d = k_{dv} * K_e$

    \item Compute the nominal view tag: ${\tt dense\_view\_tag}' = \mathcal{H}_{dvt}(K'^{dv}_d || K^o)$

    \item If ${\tt dense\_view\_tag}' \neq {\tt dense\_view\_tag}$ then {\tt ABORT}.
\end{enumerate}

\subsubsection{Full enote recovery}
\label{subsubsec:jamtis-balance-recovery-enote}

Once an enote has been flagged as `possibly owned', full enote recovery can proceed. Normal scanning will use the key derivations $K'^{dv}_d$ and $K'^{sv}_d$, while selfsend scanning will test a trial selfsend type $\tau$.

In selfsend scanning, if one $\tau$ fails then other values of $\tau$ can be tried.

The user first computes $s^{sr}_1$ and recovers the address index $j$.
\begin{itemize}
    \item \textbf{Normal scan}: This requires $K'^{dv}_d$, $K'^{sv}_d$, and the user's $s_{ct}$.

    \item \textbf{Selfsend scan}: This requires the user's $k_{vb}$ and $s_{ct}$.
\end{itemize}

\begin{enumerate}
    \item Compute the first enote secret
    \begin{itemize}
        \item \textbf{Normal scan}: $s'^{sr}_1 = \mathcal{H}_{sr1n}(K'^{dv}_d || K'^{sv}_d || K_e || \texttt{input\_context})$
        \item \textbf{Selfsend scan}: $s'^{sr}_1 = \mathcal{H}_{sr1s[\tau]}(k_{vb} || K_e || {\tt input\_context})$
    \end{itemize}

    \item Compute the nominal ciphered address index: ${c^j}' = \textrm{\tt addr\_index\_enc XOR } \mathcal{H}_{aie}(s'^{sr}_1 || K^o)$
...
\begin{itemize}
    \item \textbf{Normal scan}: This requires $s'^{sr}_1, j', s^{j'}_{gen}$, and the user's $k_{ua}$ and $K_s$.
...
\begin{enumerate}
    \item Compute the second enote secret:
        ...
        \item \textbf{Normal scan}:
            \begin{enumerate}
                \item Compute the address private key of $j'$: $k^{j'}_a = \mathcal{H}^n_{ap}(K_s || j' || s^{j'}_{gen})$
                \item Compute the secret: $s'^{sr}_2 = \mathcal{H}_{sr2n}((1/(k^{j'}_a * k_{ua})) * K_e)$
            \end{enumerate}
        \item \textbf{Selfsend scan}: $s'^{sr}_2 =\mathcal{H}_{sr2s}(k_{vb} || s'^{sr}_1)$
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Rule}: All transactions spending funds from a user's wallet must contain at least one selfsend enote.\footnote{To satisfy this rule, it may be necessary to add a dummy selfsend to a transaction if there is no change enote or explicit self-spend.}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Generate-address} ($s_{ga}, K_s, K_{ua}, K_{dv}, K_{sv}$): Generate any Jamtis address for arbitrary index $j$. Decipher the address index of any existing Jamtis address.

    \item \textbf{Dense view tag} ($k_{dv}$): Perform dense view tag checks.

    \item \textbf{Sparse view tag} ($k_{sv}$): Perform sparse view tag checks.

    \item \textbf{Find-received} ($k_{dv}, k_{sv}, s_{ga}, K_s$): Perform all view tag checks, decipher address indices, and recompute one-time pubkeys to guarantee ownership for normal enotes.

    \item \textbf{Payment validator} ($k_{dv}, k_{sv}, k_{ua}, s_{ga}, K_s$): In addition to the find-received and generate-address tier capabilities, the payment validator can view the amounts of incoming transactions. This tier cannot compute key images or view outgoing/change amounts.
...
\begin{itemize}
    \item A find-received wallet has very limited information about the user's owned enotes.
    \begin{enumerate}
        \item It can perform view tag checks on all enotes, which essentially filters down the set of enotes `possibly owned by the user'.
        \item It can uncover the nominal address tags of normal enotes, which means if the wallet knows about (or ever in the future learns about) addresses owned by the user, then it can identify all normal enotes owned by those addresses.
    \end{enumerate}

    \item A find-received wallet only has to transmit basic records of enotes that pass view tag checks and key images associated with those enotes to the user. This amounts to a substantial relative reduction in data that a user needs to access compared to the remote wallet, even if view tags are only 8 bits (a 256:1 reduction).

    \item The first step that a user needs to do to process a basic record is check the nominal address tag's hint, which is very cheap compared to the ECDH exchange required to obtain that hint. This means the user will spend very little computational time handling enotes they don't actually own, since hint checks will further filter out a significant proportion of unowned enotes (for a 16-bit hint it would be a 65536:1 reduction).

    \item The user will only look for selfsend enotes in transactions containing the key images of their owned enotes. Moreover, despite the selfsend path needing to decrypt the encrypted address tag before doing a hint check, doing so is very cheap since $s^{sr}_1$ for selfsends is derived from $k_{vb}$ instead of $K_d$. Overall this means checking for selfsends has very low relative computation cost.
\end{itemize}
...
\begin{itemize}
    \item The adversary can solve for $k_{dv}$ and $k_{sv}$ in $k_{dv} D^j_{ua} = D^j_{dv}$ and $k_{sv} D^j_{ua} = D^j_{sv}$ for use in unwrapping candidate onetime addresses to check if the unwrapped spend keys match the address's $K^j_s$.

    \item The adversary can solve for $r$ in $r D^j_{ua} = K_e$ to compute the key $r G$, which allows the $s^{sr}_2$ of normal enotes to be computed.
\end{itemize}
...
\begin{itemize}
    ...
    \item Enote ephemeral public keys use a uniformly distributed value $r$, and the discrete log of $K_e$ with respect to a public generator is never used.
